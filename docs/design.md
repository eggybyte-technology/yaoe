...
NETBIRD_DOMAIN_EXACT = ["signal.netbird.io", "stun.netbird.io", "turn.netbird.io"]
NETBIRD_PROXY_DOMAIN_EXACT = ["api.netbird.io", "app.netbird.io", "pkgs.netbird.io"]
...
SING_BOX_CN_DNS_PORT = 443
SING_BOX_CN_DNS_PATH = "/dns-query"
SING_BOX_CN_DNS_TLS_SERVER_NAME = "dns.alidns.com"
SING_BOX_REMOTE_DNS_TYPE = "https"
SING_BOX_REMOTE_DNS_SERVER = "1.1.1.1"
SING_BOX_REMOTE_DNS_PORT = 443
SING_BOX_REMOTE_DNS_PATH = "/dns-query"
...
MIHOMO_DIRECT_NAMESERVER = ["https://dns.alidns.com/dns-query", "https://doh.pub/dns-query"]
MIHOMO_FALLBACK = ["https://1.1.1.1/dns-query#PROXY", "https://8.8.8.8/dns-query#PROXY"]
MIHOMO_FALLBACK_FILTER_GEOIP = true
MIHOMO_FALLBACK_FILTER_GEOIP_CODE = "CN"
MIHOMO_FALLBACK_FILTER_GEOSITE = ["gfw"]
MIHOMO_FALLBACK_FILTER_DOMAIN = ["+.google.com", "+.youtube.com", "+.facebook.com", "+.twitter.com", "+.x.com"]
```
...
```text
yaoe render config
  -> load and validate .yaoe/yaoe.toml using full validation
  ...
  -> print one line per variant in the exact format render <variant> <path>
```
...
`100.64.0.0/10` is not represented as a user default. It is a fixed NetBird built-in direct CIDR in every generated client config and cannot be removed through `.yaoe/yaoe.toml`.
...
19. User-provided `route.direct_cidrs` entries that duplicate built-in direct CIDRs, `100.64.0.0/10`, or managed-server endpoint `/32` CIDRs are INVALID because those routes are generated explicitly by the fixed routing model.
20. Cloudflare token, account ID, delivery domain, and R2 bucket are not empty or whitespace-only.
21. `credential.vless_uuid` is a valid UUID string.
22. `credential.config_key` matches section 3.3 exactly.
23. Reality credential fields match section 3.3 exactly.
24. Reality public key derivation from `credential.reality_private_key` succeeds.
25. Placeholder values created by `yaoe init` are INVALID for effectful commands.
26. Values derived by section 0.4 are absent from user configuration.
...
`.yaoe/work/delivery/rendered-config/*` files embed delivery credentials and server facts. These files are generated work files overwritten by `yaoe render config`, `yaoe publish config`, and `yaoe publish delivery`.
...
### 5.4 `yaoe render config` Output Contract

`yaoe render config` stdout is exact after rendering and local validation. It prints one line per generated config variant in `CONFIG_VARIANTS` order:
...
10. Render platform configs to `.yaoe/work/delivery/rendered-config/` for exactly the seven variants in `CONFIG_VARIANTS` using the same renderer and validator path as `yaoe render config`.
...
Platform client configs are generated by `yaoe render config`, `yaoe publish config`, and by the config sub-step inside `yaoe publish delivery`.
...
These files are overwritten during `yaoe render config`, `yaoe publish config`, and `yaoe publish delivery`.
...
TUN is exact except `route-exclude-address` is generated from the direct CIDR construction rule below:
...
tun:
  ...
  route-exclude-address:
    ...
    - 192.168.0.0/16
    - 100.64.0.0/10
    - <configured direct CIDR>
    - <server endpoint IPv4>/32
```
...
rules:
  ...
  - DOMAIN,signal.netbird.io,DIRECT
  - DOMAIN,stun.netbird.io,DIRECT
  - DOMAIN,turn.netbird.io,DIRECT
  - DOMAIN,api.netbird.io,PROXY
  - DOMAIN,app.netbird.io,PROXY
  - DOMAIN,pkgs.netbird.io,PROXY
  - DOMAIN-SUFFIX,relay.netbird.io,DIRECT
  - DOMAIN-SUFFIX,netbird.io,DIRECT
  - DOMAIN-SUFFIX,netbird.cloud,DIRECT
  - GEOSITE,cn,DIRECT
  - GEOIP,CN,DIRECT
  - MATCH,PROXY
```

Direct CIDR construction order is built-in IPv4 CIDRs, fixed NetBird overlay CIDR, configured IPv4 direct CIDRs, and managed-server endpoint IPv4 `/32` CIDRs. Canonical duplicates introduced by merging these lists are skipped when the same canonical CIDR already exists earlier in the generated list. User-provided duplicates rejected by validation never reach the renderer.
...
10. DNS rules first route `api.netbird.io`, `app.netbird.io`, and `pkgs.netbird.io` to `remote-dns`, then route NetBird exact domains, NetBird suffix domains, and the `cn-domain` rule set to `cn-dns` before DNS final selection.
11. DNS uses global `strategy = "ipv4_only"`.
12. DNS `reverse_mapping = true`.
13. One VLESS outbound for each configured server.
14. Each VLESS outbound connects to `server.<name>.ip` and `server.<name>.port`.
15. Each VLESS outbound uses `credential.vless_uuid`.
16. Each VLESS outbound sets `flow = "xtls-rprx-vision"`.
17. Each VLESS outbound omits the sing-box `network` field so TCP and UDP destination networks remain enabled by sing-box default while the Reality transport remains VLESS over TCP.
18. Each VLESS outbound enables TLS Reality with `server_name = reality.handshake_server`.
19. Each VLESS outbound enables uTLS with `fingerprint = "chrome"`.
...
26. The next route rule sends `api.netbird.io`, `app.netbird.io`, and `pkgs.netbird.io` to `proxy` with `action = "route"` so browser web/API traffic and package repository downloads do not match the later `netbird.io` direct suffix rule. On desktop service profiles, NetBird-owned API traffic still matches the earlier process rule and remains direct.
27. The next route rules send NetBird exact domains and NetBird suffix domains to `direct`; Linux service configs use `action = "bypass"` for these rules under `auto_redirect`, while macOS and mobile configs use `action = "route"`.
28. The next route rule sends direct CIDRs to `direct`; Linux service configs use `action = "bypass"` for this rule under `auto_redirect`, while macOS and mobile configs use `action = "route"`.
29. The next route rule rejects public IPv6 with `ip_version = 6`, `action = "reject"`, `method = "default"`, and `no_drop = true`.
30. CN direct remote rule sets `cn-domain` and `cn-ipv4`.
31. Remote rule-set URLs are Gitee Release asset URLs.
32. Remote rule sets use `download_detour = "direct"`.
33. Remote rule sets use `update_interval = "1d"`.
34. CN direct route rule for `cn-domain` and `cn-ipv4` appears after the IPv6 rejection rule.
35. Final route `proxy`.
...
The fixed NetBird overlay direct CIDR is exactly:
...
The NetBird proxy exact domains are exactly:
...
The NetBird suffix domains are exactly:
...
TUN profile `mobile` contains the shared TUN fields, including `route_exclude_address`, and no process-based routing field.
...
Route profile `mobile` contains the shared route fields only and omits service-desktop process rules.
...
```json
{
  ...
  "dns": {
    ...
    "rules": [
      {
        "domain": ["api.netbird.io", "app.netbird.io", "pkgs.netbird.io"],
        "action": "route",
        "server": "remote-dns"
      },
      {
...
1. For `linux-amd64` and `linux-arm64`, insert `auto_redirect = true` and `strict_route = true` into the TUN inbound after `auto_route`, keep `route_exclude_address` after those fields, insert `auto_detect_interface = true` as the first key in `route`, render `api.netbird.io`, `app.netbird.io`, and `pkgs.netbird.io` with `action = "route"` and `outbound = "proxy"` before NetBird direct domains, and render the NetBird process/domain/domain-suffix rules plus the direct CIDR rule with `action = "bypass"` and `outbound = "direct"`.
2. For `macos-amd64` and `macos-arm64`, insert `strict_route = true` into the TUN inbound after `auto_route`, keep `route_exclude_address` after those fields, insert `auto_detect_interface = true` as the first key in `route`, render `api.netbird.io`, `app.netbird.io`, and `pkgs.netbird.io` with `action = "route"` and `outbound = "proxy"` before NetBird direct domains, and render the NetBird process/domain/domain-suffix rules plus the direct CIDR rule with `action = "route"` and `outbound = "direct"`.
3. For `ios` and `android`, use mobile profile insertion from section 9.4: do not insert `auto_detect_interface`, `auto_redirect`, `strict_route`, or `process_name` rules. The mobile route rules start with DNS hijack, sniff, `api.netbird.io`, `app.netbird.io`, and `pkgs.netbird.io` proxy routing, NetBird domain direct rules, direct CIDR route, public IPv6 rejection, CN rule-set direct route, and final `proxy`; NetBird domain rules and the direct CIDR rule use `action = "route"`.
...
7. Run `yaoe check`.
8. Run `yaoe render config` to validate all generated client profiles locally.
9. Run `yaoe publish delivery`.
10. Run `yaoe apply`.
11. Run `yaoe status`.
12. Run `yaoe health`.
...
24. State that generated configs implement IPv4 egress semantics: private/local traffic, NetBird overlay traffic, NetBird signal/STUN/TURN/relay traffic, configured direct CIDRs, managed-server endpoint IPs, and CN allowlist traffic are direct; remaining public IPv4 traffic uses proxy aggregation, including browser traffic to `api.netbird.io`.
...
- [Cloudflare 1.1.1.1 DNS over HTTPS](https://developers.cloudflare.com/1.1.1.1/encryption/dns-over-https/)
...
- [sing-box DNS over HTTPS server](https://sing-box.sagernet.org/configuration/dns/server/https/)
...
### 13.8 NetBird Overlay Compatibility

- [NetBird DNS](https://docs.netbird.io/manage/dns)
- [NetBird backend service configuration and CGNAT range](https://docs.netbird.io/manage/reverse-proxy/service-configuration)
- [NetBird relay and STUN server setup](https://docs.netbird.io/selfhosted/maintenance/scaling/set-up-external-relays)
- [NetBird architecture: Signal and Relay services](https://docs.netbird.io/about-netbird/how-netbird-works)

### 13.9 System Services, Runtime Probing, and Shell Primitives
