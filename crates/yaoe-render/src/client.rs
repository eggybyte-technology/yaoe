...
use yaoe_home::{
    CN_DNS_PORT, CN_DNS_SERVER, CN_DNS_TLS_SERVER_NAME, CN_DOMAIN_PUBLIC_ASSET, CN_DOMAIN_RULE_TAG,
    CN_IPV4_PUBLIC_ASSET, CN_IPV4_RULE_TAG, DNS_HIJACK_PORT, DNS_STRATEGY, GITEE_RELEASE_TAG,
    MIHOMO_ALLOW_LAN, MIHOMO_DEFAULT_NAMESERVER, MIHOMO_DNS_ENABLE, MIHOMO_DNS_ENHANCED_MODE,
    MIHOMO_DNS_IPV6, MIHOMO_FAKE_IP_RANGE, MIHOMO_FALLBACK, MIHOMO_FALLBACK_FILTER_GEOIP,
    MIHOMO_FALLBACK_FILTER_GEOIP_CODE, MIHOMO_GEO_AUTO_UPDATE, MIHOMO_GEO_UPDATE_INTERVAL_HOURS,
    ...
    MIHOMO_URL_TEST_URL, PUBLIC_IPV6_DENIAL_NO_DROP, REMOTE_DNS_PORT, REMOTE_DNS_SERVER,
    REMOTE_DNS_TLS_SERVER_NAME, TUN_IPV4_ADDRESS, TUN_IPV6_ADDRESS, YaoeError, YaoeResult,
    config_variant,
};
...
pub fn render_client_config(
    ...
    let doc = ClientConfigDoc {
        ...
        route: Route {
            ...
            rules: vec![
                RouteRule::DnsHijack(DnsHijackRouteRule {
                    port: DNS_HIJACK_PORT,
                    action: "hijack-dns",
                }),
                RouteRule::Sniff(SniffRouteRule { action: "sniff" }),
                RouteRule::Cidr(CidrRouteRule {
                    ip_cidr: direct_cidrs(&input.config, &server_names)?,
                    action: "route",
                    outbound: "direct",
                }),
                RouteRule::Ipv6Reject(Ipv6RejectRouteRule {
                    ip_version: 6,
                    action: "reject",
                    method: "default",
                    no_drop: PUBLIC_IPV6_DENIAL_NO_DROP,
                }),
                RouteRule::RuleSet(RuleSetRouteRule {
...
fn validate_shared_client_shape(config: &Value) -> YaoeResult<()> {
    ...
    if remote_dns.get("type").and_then(Value::as_str) != Some("tls")
    ...
    if dns_rules.len() != 1
        || dns_rules[0].get("rule_set") != Some(&serde_json::json!([CN_DOMAIN_RULE_TAG]))
    ...
    if rules.len() != 5
        || rules[0].get("port").and_then(Value::as_u64) != Some(DNS_HIJACK_PORT.into())
        || rules[0].get("action").and_then(Value::as_str) != Some("hijack-dns")
        || rules[1].get("action").and_then(Value::as_str) != Some("sniff")
        || rules[2].get("ip_cidr").and_then(Value::as_array).is_none()
        || rules[2].get("action").and_then(Value::as_str) != Some("route")
        || rules[2].get("outbound").and_then(Value::as_str) != Some("direct")
        || rules[4].get("rule_set")
            != Some(&serde_json::json!([CN_DOMAIN_RULE_TAG, CN_IPV4_RULE_TAG]))
        || rules[4].get("action").and_then(Value::as_str) != Some("route")
        || rules[4].get("outbound").and_then(Value::as_str) != Some("direct")
    {
        return Err(YaoeError::Internal(
