...
use crate::logging::{info, ok, one_line, progress, record_event as log_event, warn};
...
pub fn cmd_render_config(home: Option<&Path>, deps: &RuntimeDeps) -> YaoeResult<()> {
    progress("render config: loading configuration");
    let paths = resolve_home(home);
    let config = load_and_validate(&paths.config)?;
    ensure_home(&paths)?;
    progress("render config: checking sing-box from PATH");
    deps.local_sing_box.require_version()?;
    progress("render config: checking mihomo from PATH");
    deps.local_mihomo.require_version()?;
    progress("render config: rendering and checking seven config variants");
    render_platform_configs(
        "render config",
        &paths,
        &config,
    ...
    for platform in CONFIG_VARIANTS {
        println!(
            "render {platform} {}",
            paths.rendered_config_path(platform).display()
        );
    }
    Ok(())
}
...
fn run_health_probe_for_server(
    ...
    for attempt in 1..=HEALTH_PROBE_PORT_RETRY_LIMIT {
            ...
            .run_health_probe(&path, probe_port, &summary.server)
        {
            ...
            Err(failure) => {
                return Err(health_probe_error(config, summary, failure));
            }
        }
    }
...
fn health_probe_error(
    ...
    YaoeError::HealthProbe(format!(
        "server={} endpoint={}:{} handshake={}:{} stage={} proxy={} resolve=remote_hostname_ipv4 curl_status={} curl_exit={} local_probe_stderr_tail={} detail={}",
        summary.server,
        summary.listen_ip,
        summary.listen_port,
...
fn render_platform_configs(
    ...
    reset_rendered_config_dir(paths)?;
    let input = ClientRenderInput {
        config: config.clone(),
    };
    for platform in CONFIG_VARIANTS {
        let rendered = if platform == "clash-verge" {
            progress(format!("{command_name}:{platform}: render YAML"));
            render_clash_verge_profile(&input)?
        } else {
            progress(format!("{command_name}:{platform}: render JSON"));
            let client_platform =
        ...
        atomic_write(&path, rendered.as_bytes(), 0o600)?;
        validate_rendered_config(command_name, platform, &path, sing_box, mihomo)?;
    }
    Ok(())
}

fn reset_rendered_config_dir(paths: &HomePaths) -> YaoeResult<()> {
    let dir = paths.rendered_config_dir();
    fs::create_dir_all(&dir)
        .map_err(|e| YaoeError::State(format!("create {}: {e}", dir.display())))?;
    for entry in
        fs::read_dir(&dir).map_err(|e| YaoeError::State(format!("read {}: {e}", dir.display())))?
    {
        let entry =
            entry.map_err(|e| YaoeError::State(format!("read {} entry: {e}", dir.display())))?;
        let path = entry.path();
        let metadata = entry
            .metadata()
            .map_err(|e| YaoeError::State(format!("stat {}: {e}", path.display())))?;
        if metadata.is_file() {
            fs::remove_file(&path)
                .map_err(|e| YaoeError::State(format!("remove {}: {e}", path.display())))?;
        }
    }
    Ok(())
}
...
fn validate_rendered_config(
    ...
    if platform == "clash-verge" {
        progress(format!("{command_name}:{platform}: mihomo check YAML"));
        mihomo.check_config(path)
    } else {
        progress(format!("{command_name}:{platform}: sing-box check JSON"));
