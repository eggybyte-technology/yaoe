...
use yaoe_controller::{
    ...
    cmd_publish_config, cmd_publish_runtime, cmd_status,
};
...
fn client_config_renders_reality_urltest_and_direct_srs() {
    ...
    assert_eq!(json["dns"]["servers"][0]["type"], "tls");
    ...
    assert_eq!(
        json["dns"]["rules"][0]["rule_set"],
        serde_json::json!(["cn-domain"])
    );
    assert_eq!(json["dns"]["rules"][0]["server"], "cn-dns");
    assert_eq!(json["route"]["rules"][0]["port"], 53);
    assert_eq!(json["route"]["rules"][0]["action"], "hijack-dns");
    ...
    let cidrs = json["route"]["rules"][2]["ip_cidr"].as_array().unwrap();
    ...
    assert_eq!(json["route"]["rules"][3]["ip_version"], 6);
    assert_eq!(json["route"]["rules"][3]["action"], "reject");
    assert_eq!(json["route"]["rules"][3]["method"], "default");
    assert_eq!(json["route"]["rules"][3]["no_drop"], true);
    assert!(json["route"]["rules"][3].get("outbound").is_none());
    assert_eq!(
        json["route"]["rules"][4]["rule_set"],
...
fn clash_verge_profile_renders_mihomo_yaml_contract() {
    ...
    assert!(rendered.contains("  - IP-CIDR,127.0.0.0/8,DIRECT,no-resolve\n"));
...
impl SshTransport for FakeSsh {
    ...
    fn run_as_root_raw(&self, _: &str, command: &str, _: &str) -> YaoeResult<RemoteCommandOutput> {
        ...
        } else if command.contains("journalctl -u yaoe-") {
            "journal tail\n".to_string()
        } else {
            String::new()
        };
