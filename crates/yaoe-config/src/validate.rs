...
use yaoe_home::{
    CONFIG_KEY_LENGTH, CONFIG_KEY_RANDOM_BYTES, REALITY_PRIVATE_KEY_LENGTH,
...
use crate::model::{Config, RouteConfig, ServerConfig};
...
fn validate_route(route: &RouteConfig) -> YaoeResult<()> {
    let mut seen = HashSet::new();
    for cidr in &route.direct_cidrs {
        let parsed = cidr
            .parse::<IpNet>()
...
"#,
    ...
    fn accepts_valid_config() {
        parse_and_validate(&valid_config()).unwrap();
    }
